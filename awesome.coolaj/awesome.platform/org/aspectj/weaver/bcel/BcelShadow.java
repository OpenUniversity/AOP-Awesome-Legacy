...
public class BcelShadow extends Shadow {
	...
	final LazyMethodGen enclosingMethod;
	...
	// ---- overridden behaviour
	 ...
	 public BcelVar[] getArgVarsField() {
	        return argVars;
	   }

	    
	    public BcelVar[] getArgVars() {
	        initializeArgVars();
	        return argVars;
	   }
...
*/
	 ...
	 *   
	 *   
	 *   Store ArgN -> ... -> Store Arg0 -> 
	 *   [? hasTarget: Store TargetVar ?] -> 
	 *   Create TJP -> Store TJP -> 
	 *   ===============
	 *   StartShadInt -> 
	 *     [? argsOnStack():  
	 *       [? ConstructorCall : New->DUP ?]->
	 *       [?hasTarget Load targetVar]->
	 *       Load ArgN->...->Load Arg0 ?] ->
	 *   Body of the Shadow Interval ->
	 *   [? PreInitialization : NOP] ->
	 *   EndShadInt
	 *   ================
	 * (1) Special care is taken for ConstructorCall, PreInitialization, and 
	 *     ExceptionHandler shadows.
	 *     (1.1) Constructor call shadow:
	 *        1.1.1 (preceding) NEW and DUP instructions are deleted: see deleteNewAndDup()
	 *        1.1.2 temp variables for args of the constructor that is associated with the 
	 *              shadow are created.
	 *        1.1.3 the temp variables are initialized by values on the op. stack
	 *        (that are actual args of the constructor call).
	 *        The temp variables are available via the <code>argVars</code> field.
	 *        For 1.1.2 and 1.1.3 see initializeArgVars().
	 *     (1.2) PreInitialization shadow: the NOP instruction is inserted InsideAfter
	 *            the shadow interval.
	 *     (1.3) ExceptionHandler shadow:
	 *        1.3.1 A temp variable for an exception object is created, and
	 *              initialized from the op. stack. This inserts OutsideBefore
	 *              STORE instruction. The variable is available via the 
	 *              <code>argVars</code> field.
	 *        1.3.2 All targeters of the original start instruction of the shadow
	 *              interval (including range object for this shadow) are 
	 *              redirected to the new STORE instruction.
	 *  
	 * (2) Specializes the mungers (see BcelAdvice.specializeOn(Shadow))
	 *     2.1 If there is at least one an around advice munger, then 
	 *         (if not already done) temp variables for shadow's 
	 *          member arguments, target object, and this object are created
	 *          (see initializeForAroundClosure()).
	 *          The temp variables (except this var) are also initialized
	 *          by inserting OutsideBefore instructions that capture the 
	 *          argument/target values, and store them in the temp vars on the 
	 *          shadow's enclosing method frame.
	 *     
	 *      
	 *     2.2 For every advice munger, its pointcut residue is identified 
	 *     AND exposedState is created and initialized
	 *     
	 *     2.3 thisJoinPointVar, thisJoinPointStaticPartVar, and 
	 *     this.getEnclosingShadow().thisJoinPointStaticPartVar fields are initialized.
	 *     This imposes no change on the instruction list of the shadow's body.
	 *  
	 *  (3) Insert OutsideBefore the shadow interval instructions that
	 *      create TJP object and store it in the TJP temp variable (in the frame).
	 *      The instructions are not dependent on the current state of the stack.
	 *      they only use temp variables (and static tjp static part field
	 *      in the enclosing class).
	 *  (4) bad advice pieces are signalled (badAdvice is determined by the specializeOn
	 *      method)
	 *  
	 *  (5) if getKind().argsOnStack() (call-like shadows), then 
	 *       5.1 If shadow is ExceptionHandler AND the next instruction after the shadow
	 *           interval is InstructionConstants.POP, then we replace it with
	 *           InstructionConstants.NOP. This is because our code before the
	 *           handler already clears the stack by storing the exception in the temp var.
	 *       5.2 Otherwise, 
	 *           5.2.1 add InsideBefore instructions that load all argVars temp vars on the stack (so, that first arg is on the top).
	 *                 These instructions become a part of the shadow!
	 *           5.2.2. If targetVar !=null, add InsideBefore instruction
	 *                  that loads targetVar on the stack.      
	 *           5.2.3. If the shadow kind is a ConstructorCall, then add InsideBefore
	 *                  to the shadow interval NEW and DUP instructions 
    ...
    /** SK: This method is only used for processing constructor call join point shadows.
     * It gets over all cases of object initialization in a byte code.
     * It removes the NEW and DUP instructions of the call, and retargets their instruction
     * targeters to the instruction that follows NEW and DUP in the code. 
     * 
     * TODO: Move it to the BaseMechanism
     * */
	protected void deleteNewAndDup() {
		final ConstantPool cpg = getEnclosingClass().getConstantPool();
		int depth = 1;
		//SK: In case of constructor call, ih is a constructor invocation instr.
		//My guess is that this could be either NEW or INVOKESPECIAL <init>.
		//INVOKESPECIAL is preceded by NEW
		//First, we find NEW.
		
		InstructionHandle ih = range.getStart();

		//we make sure, that ih is really a handler of a NEW instruction
		//for that we walk up (in a prev direction) the instruction list,
		//and ignore all object initializer calls and other instructions.
		//bit of magic for those who doesn't know JVM spec... 
		while (true) {
			Instruction inst = ih.getInstruction();
			//if this is invocation of the object initializer, then we are getting
			//one level deeper
			if (inst.opcode == Constants.INVOKESPECIAL
				&& ((InvokeInstruction) inst).getName(cpg).equals("<init>")) {
				depth++;
				//otherwise, if it is an object constructor, then, we are getting
				//one level up in depth.
			} else if (inst.opcode == Constants.NEW) {
				depth--;
				if (depth == 0) break;
			}
		...
		InstructionHandle endHandle = newHandle.getNext();
		InstructionHandle nextHandle;
		if (endHandle.getInstruction() == InstructionConstants.DUP) {
			// NEW, DUP, INVOKE => INVOKE
			nextHandle = endHandle.getNext();	
			//retargeting all targeters from NEW to INVOKE
			retargetFrom(newHandle, nextHandle);
			//retargeting all targeters from DUP to INVOKE
			retargetFrom(endHandle, nextHandle);
		} else if (endHandle.getInstruction() == InstructionConstants.DUP_X1) {
            //NEW, DUP_X1, SWAP, INVOKE => ...,INVOKE
			InstructionHandle dupHandle = endHandle;
			endHandle = endHandle.getNext();
			nextHandle = endHandle.getNext();
			if (endHandle.getInstruction() == InstructionConstants.SWAP) {}
			else {
				// XXX see next XXX comment
				throw new RuntimeException("Unhandled kind of new " + endHandle);
			}
...
//			retargeting all targeters from NEW to INVOKE
			retargetFrom(newHandle, nextHandle);
//			retargeting all targeters from DUP_X1 to INVOKE
			retargetFrom(dupHandle, nextHandle);
//			retargeting all targeters from SWAP to INVOKE
			retargetFrom(endHandle, nextHandle);
		} else {
            //NEW, INVOKE => INVOKE, POP
			endHandle = newHandle;
			nextHandle = endHandle.getNext();
//			retargeting all targeters from NEW to INVOKE			
			retargetFrom(newHandle, nextHandle);
		...
		try {
			//deletes from newHandle to endHandle, inclusively.
			range.getBody().delete(newHandle, endHandle);
		} catch (TargetLostException e) {
			throw new BCException("shouldn't happen");
		}
	...
	/** SK: Retargets instructmakeFieldSetion targeters of the old instruction to the fresh instruction. */
	private void retargetFrom(InstructionHandle old, InstructionHandle fresh) {
		InstructionTargeter[] sources = old.getTargetersArray();
		if (sources != null) {
			for (int i = sources.length - 1; i >= 0; i--) {
	...
	// records advice that is stopping us doing the lazyTjp optimization
	private List badAdvice = null;
	/** SK: Adds advice into the badAdvice list. That's it! */
	public void addAdvicePreventingLazyTjp(BcelAdvice advice) {
...
//                    (FieldInstruction) getHandle.getInstruction()),
	...
	public BcelVar getTargetVarField() {
    	return targetVar;
    }
